...
#--------------------------------------------------------------------
  def __init__(self, RC, Angle = 90, R = None,  \
...
# Initial position along Z and X
    self.dz = math.cos(math.radians(self.angle))
    self.dx = math.sin(math.radians(self.angle))

    self.get_head_gate().NormalElement = Vector(0, 0, 1)
    self.get_tail_gate().NormalElement = Vector(-self.dx, 0, self.dz)

# Move to resolve own
    self.get_head_gate().PosElement = Vector(0, 0, - RC)
    self.get_tail_gate().PosElement = Vector(-self.dx * RC, 0, self.dz * RC)
#--------------------------------------------------------------------
  def get_name(self):
    return "LongElbow"
